...
public class AmmoInventoryHandler extends BattleGuiHandler implements PaginationHandler {
    public AmmoInventoryHandler(BattlePlugin plugin) {
        super(plugin);
    }
    ...
    public void getData(Player p, PlayerGui gui, BattlePagination bg, int fromIndex, int toIndex, List<ItemStack> items) {
        ...
        if(pd.isPresent()) {
            List<Map.Entry<String, Long>> x = pd.get().getInventory().getStorage(ItemType.AMMO).list();
