...
public class AdvancementRecords implements Pagination {
    @Override
    public void supply(@NotNull Player player, @NotNull View view, @NotNull SlotChain chain) {
        ...
        if(pd == null) return;
        for (Advancement adv : api.getAdvancementManager().getAdvancements()){
            if(!chain.hasNext()) break;
            if(chain.shouldSkip()) continue;
            Slot slot = chain.next();
            InfoHolder infoHolder;
            Game game = ApiProvider.consume().getArenaManager().getGame(adv);
            if(game != null){
                infoHolder = new InfoHolder("game_");
                game.inform(infoHolder);
            } else {
                infoHolder = new InfoHolder("arena_");
                adv.inform(infoHolder);
            }
            PreparedItem icon = infoHolder.compile().replace(adv.getIcon().duplicate());
            slot.setPaginationItem(icon);
            slot.setAdditionalFunction(object -> api.getArenaManager().join(object.getPlayer(), adv));
        }
    }
}
