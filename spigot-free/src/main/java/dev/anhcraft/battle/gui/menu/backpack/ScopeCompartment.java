...
public class ScopeCompartment implements Pagination {
    @Override
    public void supply(@NotNull Player player, @NotNull View view, @NotNull SlotChain chain) {
        ...
        if(pd == null) return;
        for(Map.Entry<String, Long> entry : pd.getBackpack().getStorage(ItemType.SCOPE).list()){
            ...
            slot.setAdditionalFunction(report -> {
                if(report.getEvent() instanceof InventoryClickEvent){
                    ItemChooseEvent e = new ItemChooseEvent(report.getPlayer(), ((InventoryClickEvent) report.getEvent()).getCurrentItem(), sm);
